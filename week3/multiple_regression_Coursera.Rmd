...
print("What was the 75th percentile score? 25% above this and if it's 4.6 then b is true?")
quantile(evals$score)[4]

print("Another way to calc this...")
length(subset(evals, score > 4.6)) / length(evals)

length(subset(evals, score < 3))
```
...
ggplot(data = evals, aes(x=bty_avg, y=cls_perc_eval)) +
  geom_point()
```
...
Equation for the model line is y = 0.06664 * bty_avg + 3.88034

On average, for every increase of 1 in beauty average we would expect to see an increase of 0.06664 in score.

Average beauty score appears to be a significant predictor.
...
cutoff <- data.frame( x = c(-Inf, Inf), y = 0, cutoff = factor(50) )

ggplot(m_bty, aes(x=.fitted, y=.resid)) + 
  geom_point() + geom_line(aes( x, y, linetype = cutoff ), cutoff)

ggplot(data=m_bty, aes(.resid)) + 
...
m4 = lm(score ~ rank + gender + ethnicity + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m4)$adj.r.squared

m5 = lm(score ~ rank + gender + ethnicity + language + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m5)$adj.r.squared

m5 = lm(score ~ rank + gender + ethnicity + language + age + 
    cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m5)$adj.r.squared

m6 = lm(score ~ rank + gender + ethnicity + language + age + 
