...
hr_ml <- lm(runs ~ homeruns, data=mlb11)
summary(hr_ml)
```
...
# Create a model for runs using other variables
# Step 1 - create a scatter plot
ggplot(mlb11, aes(x=hits, y=runs)) + geom_point()

ggplot(mlb11, aes(x=wins, y=runs)) + geom_point()

ggplot(mlb11, aes(x=bat_avg, y=runs)) + geom_point()

# Step 2 - fit a linear model
hit_ml <-lm(runs ~ hits, data= mlb11)
summary(hit_ml)
win_ml <-lm(runs ~ wins, data= mlb11)
summary(win_ml)
avg_ml <-lm(runs ~ bat_avg, data= mlb11)
summary(avg_ml)
...
message("HR model r squared")
summary(hr_ml)$r.squared

message("Hit model r squared")
summary(hit_ml)$r.squared

message("Wins model r squared")
summary(win_ml)$r.squared

message("Batting Avg model r squared")
summary(avg_ml)$r.squared
...
ggplot(mlb11, aes(x=new_obs, y=runs)) + geom_point()
obs_mdl <-lm(runs ~ new_obs, data= mlb11)

ggplot(mlb11, aes(x=new_slug, y=runs)) + geom_point()
slg_mdl <-lm(runs ~ new_slug, data= mlb11)

ggplot(mlb11, aes(x=new_onbase, y=runs)) + geom_point()
obp_mdl <-lm(runs ~ new_onbase, data= mlb11)

summary(obs_mdl)$r.squared
summary(slg_mdl)$r.squared
summary(obp_mdl)$r.squared
```
...
# Check the residuals
ggplot(data=obs_mdl, aes(.fitted, y = .resid)) + 
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") + 
  xlab("Fitted values") + 
  ylab("Residuals")
...
ggplot(data=obs_mdl, aes(x=.resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

# Normal probability plot
ggplot(data=obs_mdl, aes(sample=.resid)) + 
  stat_qq()
```
